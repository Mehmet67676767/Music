...
from AlyaMuzik.misc import SUDOERS, db
...
from AlyaMuzik.utils.formatters import seconds_to_min
from AlyaMuzik.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from AlyaMuzik.utils.stream.autoclear import auto_clean
from AlyaMuzik.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                await Alya.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Alya.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            # theme = await check_theme(chat_id)
            ...
            try:
                await Alya.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
