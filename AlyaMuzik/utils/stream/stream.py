...
import config
from AlyaMuzik import Carbon, YouTube, app
from AlyaMuzik.core.call import Alya
from AlyaMuzik.misc import db
from AlyaMuzik.utils.database import (
    add_active_chat,
...
from AlyaMuzik.utils.exceptions import AssistantErr
from AlyaMuzik.utils.inline.play import stream_markup, queue_markup, telegram_markup
from AlyaMuzik.utils.inline.playlist import close_markup
from AlyaMuzik.utils.pastebin import Alyabin
from AlyaMuzik.utils.stream.queue import put_queue, put_queue_index
from AlyaMuzik.utils.thumbnails import gen_thumb, gen_qthumb
...
async def stream(
    ...
    if forceplay:
        await Alya.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Alya.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Alya.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
